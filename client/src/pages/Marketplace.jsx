...
export default function Marketplace() {
    ...
    const handleOpenPublishModal = () => {
        if (!token) {
            return toast.error("¡Inicia Sesión!", {
                description: "Debes estar logeado para publicar.",
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedFiles.length === 0) return toast.error("¡Sube fotos reales!");

        setUploading(true);
        const data = new FormData();
        // ✅ REPARACIÓN: Aseguramos que TODOS los campos se agreguen al FormData
        ...
        try {
            ...
            if (res.ok) {
                toast.success("¡Mazo inyectado con éxito!");
                setShowModal(false);
                fetchItems();
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#060912] text-slate-900 dark:text-white pb-32 transition-colors duration-500">
            ...
            <div className="w-full bg-white dark:bg-slate-900/40 backdrop-blur-2xl border-b border-slate-200 dark:border-white/5 py-16 px-6 text-center relative overflow-hidden">
                ...
                <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-6xl md:text-8xl font-black italic tracking-tighter uppercase mb-4 relative z-10">
                    Market<span className="text-blue-500">Place</span>
                </motion.h1>
                <div className="flex items-center justify-center gap-2 text-slate-500 dark:text-slate-400 font-bold uppercase tracking-[0.3em] text-xs">
                    <ShieldCheck size={14} className="text-blue-500" /> Mercado Oficial de Invocadores
                </div>
            </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-16">
                    <div className="flex bg-white dark:bg-slate-900/80 p-1.5 rounded-[2rem] border border-slate-200 dark:border-white/5 w-full md:w-auto shadow-xl">
                        {["all", "imperio", "primer_bloque"].map(f => (
                            <button key={f} onClick={() => setFilter(f)} className={`flex-1 px-4 md:px-8 py-3 rounded-[1.5rem] text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-blue-600 dark:hover:text-white'}`}>
                                {f === "all" ? "🌐 Todos" : f === "imperio" ? "🏛️ Imperio" : "📜 PB"}
                            </button>
                        ))}
                    </div>
                    <button onClick={handleOpenPublishModal} className="group w-full md:w-auto px-10 py-5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:scale-105 text-white rounded-[2rem] font-black uppercase italic tracking-widest flex items-center justify-center gap-3 shadow-xl active:scale-95 transition-all">
            ...
            <AnimatePresence>
                {showModal && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{scale:0.9, opacity:0, y: 50}} animate={{scale:1, opacity:1, y: 0}} exit={{scale:0.9, opacity:0, y: 50}} className="relative w-full max-w-4xl bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-white/10 rounded-[3.5rem] p-8 shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar text-slate-900 dark:text-white">
                            <div className="flex justify-between items-start mb-8">
                                <h2 className="text-3xl font-black uppercase italic italic tracking-tighter">Publicar <span className="text-blue-500">Mazo</span></h2>
                                <button onClick={() => setShowModal(false)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-xl hover:text-blue-500 transition-colors"><X size={24} /></button>
                            </div>

                            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <label className="text-[10px] font-black uppercase text-blue-500 tracking-widest ml-4">Nombre del Mazo</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-blue-500 tracking-widest ml-4">Formato MyL</label>
                                    <div className="relative">
                                        <Layers className="absolute left-4 top-4 text-slate-400" size={18} />
                                        <select className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-4 pl-12 rounded-3xl outline-none focus:border-blue-600 font-bold appearance-none" value={formData.format} onChange={e => setFormData({...formData, format: e.target.value})}>
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4">Comuna / Ciudad</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4">Estado</label>
                                    <select className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-4 rounded-3xl outline-none focus:border-blue-600 font-bold appearance-none" value={formData.condition} onChange={e => setFormData({...formData, condition: e.target.value})}>
                                        <option value="Nuevo">NM (Como nuevo)</option>
                                        <option value="Usado">SP (Jugado)</option>
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <input type="text" required className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-4 rounded-3xl outline-none focus:border-blue-600 font-bold" placeholder="Ej: Metro Moneda o Envíos" value={formData.deliveryPoint} onChange={e => setFormData({...formData, deliveryPoint: e.target.value})} />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4">Descripción / Lista de cartas</label>
                                    <textarea className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 p-4 rounded-3xl outline-none focus:border-blue-600 font-bold h-24" placeholder="Ej: Incluye 50 cartas..." value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
                                </div>
                                <div>
                                    <label className="text-[10px] font-black uppercase text-green-600 tracking-widest ml-4">WhatsApp</label>
                                ...
                                <div>
                                    <label className="text-[10px] font-black uppercase text-pink-600 tracking-widest ml-4">Instagram</label>
                                ...
                                <div className="md:col-span-2">
                                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest ml-4">Fotos del Mazo</label>
                                    <div className="grid grid-cols-4 gap-4">
                                        ...
                                        {previews.map((src, i) => (
                                            <div key={i} className="aspect-square rounded-[2rem] overflow-hidden border border-slate-200 dark:border-white/10"><img src={src} className="w-full h-full object-cover" /></div>
                                        ))}
                                    </div>
                                </div>
                                <button disabled={uploading} className="md:col-span-2 py-6 bg-blue-600 hover:bg-blue-500 text-white rounded-3xl font-black uppercase italic tracking-widest shadow-2xl transition-all">
                                    {uploading ? "Subiendo..." : "Lanzar al Mercado"}
                                </button>
                            </form>
                        </motion.div>
                    </div>
            ...
            <AnimatePresence>
                {selectedItem && (
                    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{y: 50, opacity: 0}} animate={{y: 0, opacity: 1}} exit={{y: 50, opacity: 0}} className="relative w-full max-w-5xl bg-white dark:bg-[#0f172a] rounded-[3.5rem] overflow-hidden shadow-2xl border border-white/10 flex flex-col md:flex-row max-h-[90vh]">
                            ...
                            <div className="w-full md:w-[450px] p-8 md:p-12 overflow-y-auto custom-scrollbar flex flex-col">
                                ...
                                <h2 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900 dark:text-white leading-none mb-2">{selectedItem.title}</h2>
                                ...
                                <div className="space-y-6 mb-10 text-slate-600 dark:text-slate-300">
                                    <div className="flex items-center gap-4"><MapPin className="text-blue-500" size={24}/> <div><p className="text-[10px] font-black uppercase text-slate-400">Comuna</p><p className="font-bold">{selectedItem.location || "No especificada"}</p></div></div>
                                    <div className="flex items-center gap-4"><Truck className="text-blue-500" size={24}/> <div><p className="text-[10px] font-black uppercase text-slate-400">Entrega</p><p className="font-bold">{selectedItem.deliveryPoint || "Entrega a convenir"}</p></div></div>
                                ...
                                <div className="mt-auto pt-8 border-t border-slate-100 dark:border-white/5">
                                    ...
                                    <div className="grid grid-cols-1 gap-4">
                                        <a href={`https://wa.me/${selectedItem.whatsapp}`} target="_blank" className="w-full py-5 bg-green-500 hover:bg-green-600 text-white rounded-3xl font-black uppercase italic tracking-widest flex items-center justify-center gap-3 shadow-xl transition-all"><MessageCircle size={24}/> WhatsApp</a>
...
function MarketCard({ item, onOpen }) {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} onClick={() => onOpen(item)} className="bg-white dark:bg-slate-900/50 backdrop-blur-sm rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-200 dark:border-white/5 overflow-hidden group hover:border-blue-500/30 transition-all duration-500 shadow-2xl flex flex-col h-full cursor-pointer hover:translate-y-[-8px]">
            <div className="h-44 md:h-72 relative overflow-hidden">
                ...
                <div className="absolute top-4 left-4 px-4 py-1.5 bg-blue-600/90 backdrop-blur-md rounded-xl text-[7px] md:text-[9px] font-black uppercase text-white shadow-xl z-10">{item.format === 'imperio' ? '🏛️ Imperio' : '📜 PB'}</div>
                <div className="absolute bottom-4 right-4 px-3 py-1 bg-slate-900/80 backdrop-blur-md rounded-lg text-[6px] md:text-[8px] font-black uppercase text-white border border-white/10 z-10">{item.condition || "Usado"}</div>
            </div>
            <div className="p-5 md:p-10 flex flex-col flex-grow">
                ...
                {/* MOSTRAR COMUNA Y ENTREGA EN TARJETA */}
