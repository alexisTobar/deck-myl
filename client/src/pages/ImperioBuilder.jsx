...
export default function ImperioBuilder() {
    ...
    const handleAdd = (c) => {
        ...
        if (ex) { if (ex.cantidad < 3) setMazo(mazo.map(x => x.slug === c.slug ? { ...x, cantidad: x.cantidad + 1 } : x)); }
            ...
            const typeLabel = TIPOS_IMPERIO.find(t => t.id === Number(c.type))?.label || c.type;
                            ...
                            {cartas.map(c => {
                                ...
                                return (
                                    <div key={c.slug} className="relative cursor-pointer group" onClick={() => handleAdd(c)}>
                                        <div className={`rounded-xl overflow-hidden border-2 transition-all ${cant > 0 ? 'border-orange-500 shadow-[0_0_10px_#f97316]' : 'border-slate-800'}`}>
                                            <img src={getImg(c)} className="w-full h-auto" />
                                            {cant > 0 && <div className="absolute inset-0 bg-black/40 flex items-center justify-center"><div className="w-10 h-10 rounded-full bg-orange-600 text-white flex items-center justify-center font-bold border-2 border-white">{cant}</div></div>}
                                        </div>
                                ...
                                {mazoAgrupado[t].map(c => (
                                    <div key={c.slug} className="flex justify-between items-center py-2 border-b border-slate-800/50">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <button onClick={() => handleRemove(c.slug)} className="text-red-500 font-bold px-2">-</button>
                                            <span className="font-bold">{c.cantidad}</span>
                                            <button onClick={() => handleAdd(c)} className="text-green-500 font-bold px-2">+</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
