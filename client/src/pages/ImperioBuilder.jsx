...
export default function ImperioBuilder() {
    ...
    const handleTakeScreenshot = useCallback(async () => {
        ...
        try {
            ...
            const dataUrl = await toPng(galleryRef.current, { quality: 0.95, pixelRatio: 2, cacheBust: true, useCORS: true });
            ...
            <div className="flex-1 flex flex-col h-full relative overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
                    <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
                        {cartas.map(c => (
                            <div key={c.slug} onClick={() => handleAdd(c)} className={`cursor-pointer rounded-xl overflow-hidden border-2 transition-all transform group-hover:scale-105 ${mazo.find(x => x.slug === c.slug) ? 'border-orange-500 shadow-[0_0_15px_#f97316aa]' : 'border-slate-800'}`}>
                                <img src={getImg(c)} className="w-full" alt={c.name} />
                            </div>
                        ))}
                    </div>
                </div>
                        ...
                        <div style={{ position: 'fixed', top: '-10000px', left: '-10000px' }}>
                            <div ref={galleryRef} className="relative w-[1200px] flex flex-col bg-[#0f0a07] p-10 border border-orange-500/10 shadow-2xl overflow-hidden rounded-sm">
                                ...
                                <img src="https://raw.githubusercontent.com/alexisTobar/cartas-pb-webp/refs/heads/main/logo.png" 
                                     className="absolute top-[40%] left-[50%] -translate-x-1/2 -translate-y-1/2 opacity-[0.05] w-[600px]" alt="sello" />
                            </div>
                        </div>
                    </div>
