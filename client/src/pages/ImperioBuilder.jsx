...
import {
    ...
    Users, Star, ShieldAlert, ChevronDown
} from "lucide-react";
...
const TIPOS_IMPERIO = [
    { id: "1", label: "Aliado", icon: <Users size={14} /> },
    { id: "2", label: "Talismán", icon: <Layout size={14} /> },
    { id: "3", label: "Arma", icon: <Layout size={14} /> },
    { id: "4", label: "Tótem", icon: <Layout size={14} /> },
    { id: "5", label: "Oro", icon: <Globe size={14} /> }
];
...
export default function ImperioBuilder() {
    ...
    // ✅ Cálculo de totalCartas al inicio para evitar errores de referencia
    ...
    useEffect(() => {
        if (location.state?.deckToEdit) {
            ...
            setMazo(d.cards.map(c => ({ ...c, cantidad: c.quantity || 1, imgUrl: getImg(c), type: String(c.type) })));
        }
    }, [location.state]);

    // ✅ FETCH CARTAS: Usando la misma lógica funcional de PB
    useEffect(() => {
    ...
    const handleAdd = (c) => {
        ...
        if (copiasMismoNombre >= limit) return alert(`⚠️ Máximo ${limit} copias.`);

        const ex = mazo.find(x => x.slug === c.slug);
        if (ex) setMazo(mazo.map(x => x.slug === c.slug ? { ...x, cantidad: x.cantidad + 1 } : x));
        else setMazo([...mazo, { ...c, cantidad: 1, imgUrl: getImg(c), type: String(c.type) }]);
    };
    ...
    const handleSaveDeck = async () => {
        ...
        try {
            ...
            const res = await fetch(url, {
                ...
                body: JSON.stringify({ name: nombreMazo, cards: mazo.map(c => ({ ...c, quantity: c.cantidad })), format: formato, isPublic })
            });
    ...
    const handleTakeScreenshot = async () => {
        ...
        try {
            ...
            for (const card of mazo) {
                ...
                if (img) {
                    ctx.drawImage(img, dx, dy, 105, 147);
                    ctx.fillStyle = "#f97316";
                    if (ctx.roundRect) { ctx.beginPath(); ctx.roundRect(dx + 75, dy + 120, 32, 28, 8); ctx.fill(); }
                    else ctx.fillRect(dx + 75, dy + 120, 32, 28);
                    ctx.fillStyle = "white"; ctx.font = "bold 16px Arial";
                    ctx.fillText(`x${card.cantidad}`, dx + 79, dy + 140);
                }
        ...
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#F8FAFC] dark:bg-[#0f0a07] text-slate-900 dark:text-white overflow-hidden transition-colors duration-500">
            <div className="flex-1 flex flex-col h-full relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row gap-3">
                        <div className="relative flex-1">
                            <select value={edicionSeleccionada} onChange={(e) => { setEdicionSeleccionada(e.target.value); setBusqueda(""); }}
                                className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-blue-600 dark:text-orange-400 p-2.5 rounded-xl appearance-none font-black outline-none cursor-pointer text-sm"
                            >
                        ...
                        <input type="text" placeholder="Búsqueda..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="flex-1 p-2.5 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 text-sm outline-none focus:border-blue-500 dark:focus:border-orange-500 font-bold transition-all" />
                        <select value={razaSeleccionada} onChange={(e) => setRazaSeleccionada(e.target.value)} className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-orange-500/30 p-2 rounded-xl text-[11px] font-black outline-none"><option value="">Raza...</option>{RAZAS_IMPERIO.map(r => <option key={r} value={r}>{r}</option>)}</select>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 custom-scrollbar pb-24 md:pb-4" ref={gridContainerRef}>
                    {loading ? <div className="text-center mt-20 animate-pulse text-blue-600 dark:text-orange-500 font-black uppercase italic">Invocando...</div> : (
                                ...
                                return (
                                    <div key={c.slug} className="relative cursor-pointer group" onClick={() => handleAdd(c)}>
                                        <div className={`rounded-xl overflow-hidden border-2 transition-all duration-300 transform hover:scale-105 ${cant > 0 ? 'border-blue-600 dark:border-orange-500 shadow-xl' : 'border-slate-200 dark:border-slate-800'} ${c.restriction === 'banned' ? 'opacity-40 grayscale' : ''}`}>
                                            ...
                                            {cant > 0 && <div className="absolute inset-0 bg-blue-600/20 dark:bg-black/40 flex items-center justify-center"><div className="w-10 h-10 rounded-full bg-blue-600 dark:bg-orange-500 text-white flex items-center justify-center font-black text-xl border-2 border-white">{cant}</div></div>}
                                        </div>
                                        <button onClick={(e) => { e.stopPropagation(); setCardToZoom(c); }} className="absolute top-1.5 right-1.5 bg-white/80 dark:bg-black/60 backdrop-blur-md text-slate-900 dark:text-white w-7 h-7 rounded-lg flex items-center justify-center border border-slate-200 dark:border-white/20 hover:bg-blue-600 dark:hover:bg-orange-600 transition-all"><Search size={14} strokeWidth={3} /></button>
                                    </div>
                                );
                            })}
                        </div>
            ...
            <div className="hidden md:flex w-85 border-l border-slate-200 dark:border-white/10 flex-col h-screen bg-white dark:bg-[#0f0a07] shadow-2xl transition-colors">
                <div className="p-5 border-b border-slate-200 dark:border-orange-500/30 bg-slate-50 dark:bg-slate-900/50 backdrop-blur-md font-black text-slate-900 dark:text-orange-500 uppercase flex justify-between items-center shadow-sm">
                    <div className="flex items-center gap-2"><Layout size={18} /><span className="italic">Grimorio Imperio</span></div>
                    <div className={`px-3 py-1 rounded-full text-xs font-black border ${totalCartas === 50 ? 'bg-blue-600 dark:bg-orange-500 text-white' : 'bg-slate-100 dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500'}`}>{totalCartas} / 50</div>
                </div>
                
                <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
                    {ORDER_TYPES.map(t => mazoAgrupado[t] && (
                        <div key={t}>
                            <div className="flex items-center gap-2 mb-3"><div className="h-[2px] flex-1 bg-gradient-to-r from-blue-600 dark:from-orange-600/50 to-transparent"></div><h3 className="text-blue-600 dark:text-orange-500 text-[11px] font-black uppercase italic px-2">{t}</h3></div>
                            <div className="space-y-2">
                                {mazoAgrupado[t].map(c => (
                                    <div key={c.slug} className="flex justify-between items-center text-sm py-2.5 px-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 group hover:border-blue-400 dark:hover:border-orange-500/30 transition-all shadow-sm">
                                        <div className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer" onClick={() => setCardToZoom(c)}>
                                            <div className="bg-white dark:bg-slate-800 text-blue-600 dark:text-orange-400 w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs border dark:border-0">{c.cantidad}</div>
                ...
                <div className="p-5 bg-white dark:bg-slate-900/80 backdrop-blur-xl border-t border-slate-200 dark:border-white/5 flex flex-col gap-3 shadow-inner">
                    <button onClick={simularMano} className="w-full bg-slate-100 dark:bg-slate-800 hover:bg-indigo-600 text-slate-700 dark:text-white py-3 rounded-2xl font-black text-[11px] uppercase active:scale-95 flex items-center justify-center gap-2 border border-slate-200 dark:border-white/5 shadow-sm transition-all"><Eye size={16} /> Mano de Prueba</button>
            ...
            {modalGuardarOpen && (
                <div className="fixed inset-0 bg-slate-950/90 z-[500] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in" onClick={() => setModalGuardarOpen(false)}>
                    <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] w-full max-w-sm border border-slate-200 dark:border-slate-700 shadow-2xl" onClick={e => e.stopPropagation()}>
                        <h3 className="text-2xl font-black mb-8 uppercase text-blue-600 dark:text-orange-500 tracking-tighter italic text-center leading-none">Archivar Estrategia</h3>
