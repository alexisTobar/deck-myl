...
export default function ImperioBuilder() {
    ...
    const simularMano = () => {
        ...
        setTimeout(() => {
            ...
            setManoPrueba(barajaParaBarajar.slice(0, 8));
        }, 50);
    };
            ...
            <AnimatePresence>
                {manoPrueba.length > 0 && (
                    <div className="fixed inset-0 bg-slate-950/95 z-[300] flex flex-col items-center justify-center p-4 backdrop-blur-xl overflow-hidden">
                        <motion.h3 initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="text-xl md:text-2xl font-black text-blue-500 dark:text-orange-500 uppercase italic mb-8 tracking-widest text-center flex items-center gap-3"><Sparkles className="animate-pulse text-yellow-400" size={24} /> Mano Inicial Imperio</motion.h3>
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-start max-w-7xl px-4 overflow-y-auto max-h-[70vh]">
                            ...
                            {oroInicialSlug && mazo.find(c => c.slug === oroInicialSlug) && (
                                <div className="flex flex-col items-center gap-4 bg-yellow-500/5 p-6 rounded-[2.5rem] border border-yellow-500/20 w-full md:w-auto">
                                    <span className="text-[11px] font-black text-yellow-500 uppercase tracking-[0.2em] bg-yellow-500/10 px-4 py-1 rounded-full border border-yellow-500/30 text-center">Oro Inicial</span>
                                    <motion.div 
