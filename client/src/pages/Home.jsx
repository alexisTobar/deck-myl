...
export default function HomePortal() {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-[#0A0C10] dark:via-[#0f172a] dark:to-[#0A0C10] flex flex-col items-center font-sans text-slate-900 dark:text-white selection:bg-blue-100 dark:selection:bg-blue-900/30 overflow-x-hidden transition-colors duration-500">
            ...
            {/* HERO SECTION */}
            <header className="w-full max-w-7xl px-4 md:px-6 pt-16 md:pt-24 pb-12 text-center">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-blue-600/5 dark:bg-blue-400/10 border border-blue-600/10 dark:border-blue-400/20 rounded-full mb-8">
                ...
                <div className="flex justify-center mb-8">
                    <img src={LOGO_NEGRO} alt="Logo" className="w-[85vw] max-w-[500px] dark:hidden" />
                    <img src={LOGO_BLANCO} alt="Logo" className="w-[85vw] max-w-[500px] hidden dark:block" />
                </div>
                <p className="text-slate-500 dark:text-slate-400 text-base md:text-xl max-w-2xl mx-auto font-medium opacity-80">Análisis masivo de estrategias de la comunidad.</p>
            </header>
            ...
            <section className="w-full max-w-7xl px-6 mb-24">
                <div className="flex items-center gap-3 mb-10 border-b border-slate-200 dark:border-white/10 pb-6">
                    ...
                    <h3 className="text-2xl font-black uppercase italic">Top 10 Primer Bloque</h3>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-6">
                    {loading ? [...Array(5)].map((_, i) => <div key={i} className="h-64 bg-slate-800/20 animate-pulse rounded-3xl" />) : 
                        pbTrending.map((card, idx) => <MetaCard key={idx} card={card} index={idx} onClick={() => { setSelectedMetaCard(card); setShowMetaModal(true); }} />)
                    }
                </div>
            </section>
            ...
            <section className="w-full max-w-7xl px-6 mb-32">
                <div className="flex items-center gap-3 mb-10 border-b border-slate-200 dark:border-white/10 pb-6">
                    ...
                    <h3 className="text-2xl font-black uppercase italic">Top 10 Imperio</h3>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-6">
                    {loading ? [...Array(5)].map((_, i) => <div key={i} className="h-64 bg-slate-800/20 animate-pulse rounded-3xl" />) : 
                        impTrending.map((card, idx) => <MetaCard key={idx} card={card} index={idx} onClick={() => { setSelectedMetaCard(card); setShowMetaModal(true); }} />)
                    }
                </div>
            </section>
            ...
            <AnimatePresence>
                {showMetaModal && selectedMetaCard && (
                    <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative w-full max-w-4xl bg-white dark:bg-[#0f172a] border border-white/10 rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col md:flex-row">
                            <button onClick={() => setShowMetaModal(false)} className="absolute top-6 right-6 z-20 p-2 bg-slate-800 rounded-full text-white"><X size={20} /></button>
                            
                            <div className="w-full md:w-1/2 p-8 bg-slate-100 dark:bg-black/20 flex items-center justify-center">
                                <img src={selectedMetaCard.imgUrl || selectedMetaCard.img} className="w-full max-w-[240px] rounded-2xl shadow-2xl border-4 border-white/5" alt="card" />
                            </div>

                            <div className="w-full md:w-1/2 p-10 flex flex-col justify-center">
                                <span className="text-[10px] font-black uppercase text-blue-500 tracking-widest mb-2">Deep Scan Analysis</span>
                                <h3 className="text-3xl font-black uppercase italic leading-none mb-6">{selectedMetaCard.name}</h3>
                                
                                <div className="p-4 bg-blue-500/10 rounded-2xl border border-blue-500/20 mb-8 text-center">
                                    ...
                                    <ResponsiveContainer>
                                        <PieChart>
                                            ...
                                            <RechartsTooltip 
                                                ...
                                                itemStyle={{ color: '#fff' }}
                                            />
                                            <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ fontSize: '10px', fontWeight: '900', textTransform: 'uppercase', paddingTop: '15px' }} />
                                        </PieChart>
                                    </ResponsiveContainer>
                                </div>
                                        ...
                                        {selectedMetaCard.featuredDecks?.map((deck, i) => (
                                            <button 
                                                ...
                                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all border flex items-center gap-2
                                                    ${deck.isPublic 
                                                        ? 'bg-green-500/10 border-green-500/30 text-green-500 hover:bg-green-600 hover:text-white' 
            ...
            <section className="w-full max-w-7xl px-6 mb-32">
                <div className="relative bg-gradient-to-r from-blue-600 to-indigo-700 rounded-[3rem] p-1 shadow-2xl">
                    <div className="bg-white dark:bg-[#0A0C10] rounded-[2.8rem] p-12 flex flex-col md:flex-row items-center gap-10">
                        <div className="flex-1">
                            ...
                            <h2 className="text-5xl md:text-7xl font-black uppercase italic mb-6">Juegos <span className="text-blue-600">Vikingos</span></h2>
                            <p className="text-slate-500 dark:text-slate-400 text-lg mb-8 max-w-lg font-medium italic">Encuentra las cartas más codiciadas y los últimos lanzamientos de Mitos y Leyendas.</p>
                            <a href="https://www.juegosvikingos.cl" target="_blank" rel="noreferrer" className="px-8 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase italic text-xs flex w-fit gap-3 hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/30">Visitar Tienda <ExternalLink size={18} /></a>
                        </div>
                        <div className="flex-1"><motion.img animate={{ y: [0, -20, 0] }} transition={{ duration: 4, repeat: Infinity }} src={VIKINGO_LOGO} className="w-full max-w-[280px] drop-shadow-[0_20px_50px_rgba(37,99,235,0.4)]" /></div>
                    </div>
                </div>
            </section>
            ...
            <section className="w-full max-w-7xl px-6 mb-32">
                <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-4">
                    <div className="text-center md:text-left">
                        <h3 className="text-3xl font-black text-slate-900 dark:text-white uppercase italic flex items-center gap-3"><Instagram className="text-pink-600" /> Red de Invocadores</h3>
                ...
                <div className="flex gap-8 overflow-x-auto no-scrollbar py-6">
                    {players.map((p, i) => (
                        <motion.a whileHover={{ scale: 1.1, rotate: 5 }} key={i} href={p.instagram} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-3 min-w-[100px]">
                            <div className="w-24 h-24 rounded-full p-[3px] bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 shadow-xl"><div className="w-full h-full rounded-full border-[4px] border-white dark:border-[#0f172a] overflow-hidden"><img src={p.logo || "https://via.placeholder.com/150"} className="w-full h-full object-cover" /></div></div>
            ...
            <AnimatePresence>
                {showPlayerModal && (
                    <div className="fixed inset-0 z-[1200] flex items-center justify-center p-4">
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowPlayerModal(false)} className="absolute inset-0 bg-black/90" />
                        <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="relative bg-[#0f172a] border border-white/10 p-10 rounded-[3.5rem] w-full max-w-lg text-center">
                            ...
                            <form onSubmit={handleSavePlayer} className="space-y-4">
                                <input type="text" required className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-sm font-bold text-white outline-none focus:border-pink-600 transition-all" placeholder="Nombre" value={newPlayerData.name} onChange={e => setNewPlayerData({...newPlayerData, name: e.target.value})} />
                                <input type="url" required className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-sm font-bold text-white outline-none focus:border-pink-600 transition-all" placeholder="Instagram URL" value={newPlayerData.instagram} onChange={e => setNewPlayerData({...newPlayerData, instagram: e.target.value})} />
                                <input type="text" className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-sm font-bold text-white outline-none focus:border-pink-600 transition-all" placeholder="Logo URL" value={newPlayerData.logo} onChange={e => setNewPlayerData({...newPlayerData, logo: e.target.value})} />
                                <button type="submit" className="w-full py-5 bg-pink-600 text-white rounded-3xl text-xs font-black uppercase tracking-[0.2em] shadow-lg">Inyectar Leyenda</button>
                            </form>
                        </motion.div>
                    </div>
                )}
            ...
            {/* CARACTERÍSTICAS */}
            <div className="w-full bg-white/60 dark:bg-[#0A0C10]/60 backdrop-blur-xl border-y dark:border-white/10 py-24 mb-20">
...
function MetaCard({ card, index, onClick }) {
    return (
        <motion.div whileHover={{ y: -8 }} onClick={onClick} className="cursor-pointer group bg-white/40 dark:bg-white/5 backdrop-blur-md border border-white dark:border-white/10 p-3 rounded-[1.5rem] hover:shadow-2xl transition-all duration-500 shadow-xl shadow-black/5">
            <div className="aspect-[3/4] bg-slate-100 dark:bg-slate-800 rounded-xl mb-3 overflow-hidden relative text-center">
            ...
            <div className="text-center px-1"><h4 className="text-[10px] md:text-xs font-black truncate uppercase mb-1">{card.name}</h4><div className="flex items-center justify-center gap-1 text-blue-600 dark:text-blue-400 font-black text-[9px] uppercase"><BarChart3 size={10} /> Análisis</div></div>
        </motion.div>
    );
}
...
function FormatCard({ title, desc, img, icon, onClick, delay }) {
    return (
        <div onClick={onClick} className={`group cursor-pointer bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-white/10 overflow-hidden hover:shadow-[0_20px_50px_rgba(37,99,235,0.15)] transition-all flex flex-col animate-in fade-in slide-in-from-bottom-10 ${delay}`}>
...
function Feature({ icon, title, text }) {
    return (
        <div className="text-center flex flex-col items-center px-4">
            ...
            <h4 className="text-sm font-black uppercase tracking-widest mb-4 italic">{title}</h4>
            <p className="text-slate-500 dark:text-slate-400 text-xs font-medium leading-relaxed max-w-[220px] opacity-70">{text}</p>
        </div>
    );
}