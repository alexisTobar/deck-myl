...
export default function HomePortal() {
    ...
    useEffect(() => {
        const fetchTrendingData = async () => {
            ...
            try {
                ...
                // ✅ 2. CARGAR CARTAS CON RESTRICCIÓN DESDE LA API (REPARADO PARA MOSTRAR TODAS)
                ...
                if (resCards.ok) {
                    ...
                    // Filtramos todas las que tengan alguna restricción
    ...
    const getRestrictionStyle = (res) => {
        if (res === "banned") return "bg-red-600";
        if (res === "limited1") return "bg-orange-600";
        if (res === "limited2") return "bg-yellow-500 text-black";
        return "bg-slate-700";
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50 dark:from-[#0A0C10] dark:via-[#0f172a] dark:to-[#0A0C10] flex flex-col items-center font-sans text-slate-900 dark:text-white selection:bg-blue-100 dark:selection:bg-blue-900/30 overflow-x-hidden transition-colors duration-500">
            ...
            {/* ✅ SECCIÓN REPARADA: CARRUSEL DE BANEADAS LLAMANDO CARTAS UNA A UNA DE LA BD */}
            {bannedCards.length > 0 && (
                <motion.section initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="w-full mb-24 relative overflow-hidden bg-slate-100/50 dark:bg-white/5 py-12">
                    ...
                    <div className="relative flex overflow-hidden cursor-grab active:cursor-grabbing" ref={carouselRef}>
                        <motion.div 
                        ...
                        >
                            {/* Mostramos las cartas de la base de datos una a una */}
                            {[...bannedCards, ...bannedCards].map((card, i) => (
                                <div key={`${card._id}-${i}`} className="w-44 md:w-60 shrink-0 select-none">
                                    <div className="relative group/card bg-slate-200 dark:bg-slate-800 rounded-[2rem] overflow-hidden shadow-2xl border border-white/10 aspect-[3/4.2]">
                                        ...
                                        />
                                        {/* Badge de restricción dinámico */}
                                        <div className={`absolute top-4 right-4 px-3 py-1.5 ${getRestrictionStyle(card.restriction)} text-white text-[9px] font-black uppercase rounded-xl shadow-2xl z-10 border border-white/20`}>
                                            {card.restriction?.replace('limited', 'Copia ')}
                                        </div>
                                        {/* Info overlay */}
                                        <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent opacity-0 group-hover/card:opacity-100 transition-opacity flex flex-col items-center justify-end p-6 text-center">
                                            <p className="text-white text-xs font-black uppercase italic tracking-tighter">{card.name}</p>
                                            <p className="text-blue-400 text-[8px] font-bold uppercase tracking-widest mt-1">{card.edition?.replace('_', ' ')}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
            ...
            <motion.section initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-full max-w-7xl px-6 mb-32">
                <div className="flex flex-col md:flex-row items-center justify-between mb-10 gap-4">
                    ...
                    <button onClick={() => setShowPlayerModal(true)} className="px-6 py-3 bg-white dark:bg-white/5 border border-pink-600/30 rounded-2xl font-black text-[10px] uppercase text-pink-600 hover:bg-pink-600 hover:text-white transition-all shadow-lg flex items-center gap-2 active:scale-95">
            ...
            <AnimatePresence>
                {showPlayerModal && (
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ scale: 0.8, opacity: 0, y: 100 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.8, opacity: 0, y: 100 }} className="relative w-full max-w-lg bg-[#0f172a] border border-white/10 rounded-[3.5rem] shadow-2xl p-10 text-white text-center">
                            ...
                            <form onSubmit={handleSavePlayer} className="space-y-4 text-center">
                                ...
                                <button type="submit" className="w-full py-5 bg-pink-600 text-white rounded-3xl text-xs font-black uppercase italic tracking-[0.2em] shadow-lg hover:scale-105 active:scale-95 transition-all">Inyectar Leyenda</button>
                            </form>
                        </motion.div>
                    </div>
                )}
            ...
            {/* CARACTERÍSTICAS */}
            <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="w-full bg-white/60 dark:bg-[#0A0C10]/60 backdrop-blur-xl border-y dark:border-white/10 py-24 mb-20 transition-all text-center">
