...
export default function ImperioBuilder() {
    ...
    // ✅ REPARADO: Lógica de carga para Edición con normalización de PB
    useEffect(() => {
    ...
    // ✅ REPARADO: handleAdd funcional igual a PB para evitar duplicados
    const handleAdd = (c) => {
        setMazo(prevMazo => {
            const exIndex = prevMazo.findIndex(x => x.slug === c.slug);
            const totalActual = prevMazo.reduce((acc, card) => acc + card.cantidad, 0);

            if (totalActual >= 50 && exIndex === -1) {
                alert("Mazo lleno (50/50)");
                return prevMazo;
            }

            if (exIndex !== -1) {
                const newMazo = [...prevMazo];
        ...
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0f0a07] text-white overflow-hidden">
            <div className="flex-1 flex flex-col h-full relative overflow-hidden border-r border-white/5">
                <div className="bg-slate-900/80 border-b border-orange-500/20 p-3 flex justify-between items-center px-4 shadow-xl">
                    ...
                    <h2 className="text-xs font-black uppercase text-orange-500 tracking-widest leading-none italic flex items-center gap-2"><Star size={14}/> Imperio Builder</h2>
