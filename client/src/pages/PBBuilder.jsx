...
export default function PBBuilder() {
    ...
    // ✅ Estadísticas para la Infografía
    ...
    // ✅ SOLUCIÓN: Captura mejorada con estadísticas y logo
    const handleTakeScreenshot = useCallback(async () => {
        if (!galleryRef.current) return;
        setGuardando(true);
        try {
            await new Promise(r => setTimeout(r, 600)); // Esperar renderizado
            ...
            link.href = dataUrl; 
            link.click();
        } catch (err) { alert('Error captura móvil'); } finally { setGuardando(false); }
    }, [nombreMazo]);
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0c0e14] text-white overflow-hidden">
            ...
            {/* MODAL GALERÍA HD CON LOGO SELLO DE AGUA Y ESTADÍSTICAS */}
            {modalMazoOpen && (
                <div className="fixed inset-0 bg-black z-[120] flex flex-col overflow-hidden animate-fade-in text-white">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 pb-24 md:p-10 bg-[#0c0e14] flex flex-col items-center">
                        ...
                        {/* ✅ ÁREA DE CAPTURA HD (OCULTA FUERA DE PANTALLA PARA EL MÓVIL) */}
                             ...
                             <div ref={galleryRef} className="relative w-[1200px] flex flex-col bg-[#0c0e14] p-10 border-4 border-yellow-600 shadow-2xl overflow-hidden rounded-sm">
                                 ...
                                 <div className="flex justify-between items-end mb-8 border-b-2 border-yellow-500/20 pb-4 relative z-10 text-white font-black italic">
                                    <div className="flex items-center gap-4">
                                        <img src="https://raw.githubusercontent.com/alexisTobar/cartas-pb-webp/refs/heads/main/logo.png" crossOrigin="anonymous" className="w-16 h-16 object-contain" alt="logo" />
                                        <div>
                                 ...
                                 {/* ✅ Pie de página con Recuadros de Cantidades */}
                                 <div className="grid grid-cols-5 gap-3 relative z-10 text-center font-black">
                                    {ORDER_TYPES.map(t => (
                                        <div key={t} className="flex flex-col items-center justify-center bg-slate-900 p-6 rounded-3xl border border-yellow-600/20 shadow-inner">
                                            <span className="text-4xl font-black text-white leading-none mb-1">{statsForExport.counts[t] || 0}</span>
                                            <span className="text-[8px] uppercase font-black text-slate-500 tracking-widest uppercase">{t}</span>
                                        </div>
                                    ))}
                                 </div>
                             </div>
                        ...
                        {/* Texto de aviso en el modal */}
                        <div className="hidden md:block py-10 bg-slate-900/40 rounded-3xl border border-white/5 p-8 text-center max-w-2xl">
                             <LayoutGrid size={40} className="mx-auto text-yellow-500 mb-4" />
                             <h3 className="text-xl font-black uppercase italic">Generador de Infografía</h3>
                             <p className="text-slate-400 mt-2">La imagen incluirá tu mazo completo organizado por tipos, cantidades y el sello oficial.</p>
                        </div>
                    </div>
