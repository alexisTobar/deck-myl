...
const TIPOS_PB = [
    ...
    { id: "Tótem", icon: <Box size={14} />, label: "Tótem", color: "border-emerald-600 text-emerald-500" },
    { id: "Oro", icon: <Coins size={14} />, label: "Oro", color: "border-amber-400 text-amber-300" }
];
...
export default function PBBuilder() {
    ...
    const canvasPreviewRef = useRef(null); // ✅ Ref para ver la imagen en el modal
    ...
    // ✅ Estadísticas para la Infografía (Tipos y Curva)
    const stats = useMemo(() => {
    ...
    // ✅ SOLUCIÓN: LÓGICA DE RENDERIZADO EN CANVAS (PREVIEW + DOWNLOAD)
    const generateInfographic = async (isDownload = false) => {
        ...
        // 1. Logo Sello de Agua y Cabecera
        ...
        // 3. Panel Curva de Oro (Derecha)
        const curveX = 1250;
        const curveY = 650;
        ctx.fillStyle = "#1e293b";
        ctx.fillRect(curveX - 20, 580, 340, 350);
        ctx.fillStyle = "white";
        ...
        // 4. Panel Distribución (Inferior Recuadros)
        ctx.fillStyle = "#1e293b"; ctx.fillRect(60, 960, 1150, 210);
        ctx.fillStyle = "white"; ctx.font = "bold 24px Arial"; ctx.fillText("Distribución del Mazo", 90, 1005);
        let iconX = 100;
        const types = [
        ...
        types.forEach(t => {
            ctx.fillStyle = "#334155"; ctx.fillRect(iconX, 1025, 180, 125);
            ctx.fillStyle = "#eab308"; ctx.font = "bold 45px Arial"; ctx.fillText(t.val, iconX + 60, 1085);
    ...
    // Actualizar Canvas cuando se abre el modal
    useEffect(() => {
        if (modalMazoOpen) setTimeout(() => generateInfographic(false), 300);
    }, [modalMazoOpen, mazo]);
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0c0e14] text-white overflow-hidden">
            ...
            <div className="hidden md:flex w-85 border-l border-white/10 flex-col h-screen bg-slate-950">
                                ...
                                {mazoAgrupado[t].map(c => (
                                    <div key={c.slug} className="flex justify-between items-center text-sm py-2 px-3 bg-white/5 rounded-xl border border-white/5 group hover:bg-yellow-600/10 transition-all cursor-pointer">
                                        ...
                                        <div className="flex gap-1">
                                            <button onClick={() => handleAdd(c)} className="w-7 h-7 flex items-center justify-center bg-yellow-500/20 hover:bg-yellow-500 text-yellow-500 rounded-lg"><Plus size={14}/></button>
                ...
                <div className="p-5 bg-slate-900/80 border-t border-white/5 flex flex-col gap-3 shadow-2xl">
                    <button onClick={() => setModalMazoOpen(true)} className="w-full bg-slate-800 hover:bg-blue-600 text-white py-3 rounded-2xl font-black text-[11px] uppercase transition-all flex items-center justify-center gap-2 border border-white/5 shadow-lg"><Eye size={16} /> Ver Infografía HD</button>
            ...
            {/* MODAL INFOGRAFÍA HD (SOLUCIONADO) */}
            {modalMazoOpen && (
                <div className="fixed inset-0 bg-black z-[120] flex flex-col overflow-hidden animate-fade-in text-white">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 flex flex-col items-center bg-[#0c0e14]">
                        <div className="bg-slate-900/60 p-4 rounded-2xl border border-white/5 mb-4 text-center">
                            <p className="text-slate-400 text-sm italic font-bold">Generando previsualización horizontal... (Diseño Pro)</p>
                        </div>
                        {/* ✅ Aquí está la solución: El Canvas es visible en el modal */}
                    ...
                    <div className="p-6 bg-slate-900 border-t border-white/5 flex justify-center gap-4 shadow-2xl">
                        <button onClick={() => generateInfographic(true)} disabled={guardando} className="w-full md:w-auto bg-yellow-600 hover:bg-yellow-500 text-black px-12 py-4 rounded-2xl font-black shadow-xl flex items-center justify-center gap-3 uppercase text-sm tracking-widest active:scale-95 transition-all">
