...
export default function PBBuilder() {
    ...
    // ✅ Capturamos la edición inicial desde el state de navegación
    ...
    // Lógica para exportar cantidades en la infografía
    ...
    const handleTakeScreenshot = useCallback(async () => {
        ...
        try {
            ...
            const link = document.createElement('a'); link.download = `WD_PB_${nombreMazo || "Deck"}.png`; link.href = dataUrl; link.click();
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0c0e14] text-white overflow-hidden">
                                ...
                                {mazoAgrupado[t].map(c => (
                                    <div key={c.slug} className="flex justify-between items-center text-sm py-2.5 px-4 bg-white/5 backdrop-blur-sm rounded-2xl border border-white/5 group hover:bg-yellow-600/10 hover:border-yellow-500/30 transition-all duration-300 shadow-sm relative overflow-hidden">
                                        ...
                                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                            <button onClick={() => handleAdd(c)} className="w-8 h-8 flex items-center justify-center bg-yellow-500/20 hover:bg-yellow-500 text-yellow-500 rounded-xl transition-all active:scale-90"><Plus size={16} strokeWidth={3} /></button>
                                            <button onClick={() => handleRemove(c.slug)} className="w-8 h-8 flex items-center justify-center bg-red-500/20 hover:bg-red-600 text-red-400 rounded-xl transition-all active:scale-90"><Minus size={16} strokeWidth={3} /></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-800 p-2 pb-4 z-50 flex items-center justify-between shadow-2xl">
                <div className="flex flex-col px-3"><span className="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">Deck Total</span><span className={`text-lg font-black ${totalCartas === 50 ? 'text-green-500' : 'text-white'}`}>{totalCartas}/50</span></div>
                ...
                <div className="fixed inset-0 bg-black z-[120] flex flex-col overflow-hidden animate-fade-in text-white">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 pb-24 md:p-10 bg-[#0c0e14] flex flex-col items-center">
                        ...
                        <div className="hidden md:block">
                            <div ref={galleryRef} className="relative w-[1200px] flex flex-col bg-[#0c0e14] p-10 border border-yellow-500/10 shadow-2xl overflow-hidden rounded-sm">
                                <div className="flex justify-between items-end mb-8 border-b-2 border-yellow-500/20 pb-4 relative z-10 text-white font-black italic">
                                    <div>
                                        ...
                                        <h1 className="text-6xl uppercase tracking-tighter leading-none mt-1 leading-none">{nombreMazo || "Estrategia Ancestral"}</h1>
                                    </div>
                                    <div className="text-right">
                                        <div className="text-4xl leading-none tracking-tighter">{totalCartas} <span className="text-yellow-500 text-xl">CARTAS</span></div>
                                ...
                                <div className="grid grid-cols-5 gap-3 relative z-10">
                                    {TIPOS_PB.map(t => (
                                        <div key={t.id} className="flex flex-col items-center justify-center bg-slate-900/80 p-6 rounded-3xl border border-white/5 shadow-inner">
                                            <span className="text-3xl mb-1 text-yellow-500">{t.icon}</span>
                                            <span className="text-4xl font-black text-white">{statsForExport.counts[t.id] || 0}</span>
                                            <span className="text-[8px] uppercase font-black text-slate-500 mt-1">{t.label}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="absolute top-[40%] left-[50%] -translate-x-1/2 -translate-y-1/2 opacity-[0.03] pointer-events-none text-yellow-500"><Shield size={650} /></div>
                            </div>
                        </div>

                        {/* Clon para captura móvil */}
                        <div className="md:hidden" style={{ position: 'fixed', top: '-10000px', left: '-10000px' }}>
                             <div className="relative w-[1200px] flex flex-col bg-[#0c0e14] p-10 border border-yellow-500/10 shadow-2xl overflow-hidden rounded-sm" 
                                 ...
                                 <div className="flex justify-between items-end mb-8 border-b-2 border-yellow-500/20 pb-4 text-white font-black italic">
                                    <div><h1 className="text-6xl uppercase tracking-tighter mt-1">{nombreMazo}</h1></div>
                                    <div className="text-right"><div className="text-4xl">{totalCartas} CARTAS</div></div>
                                 </div>
                    ...
                    <div className="p-6 bg-slate-900 border-t border-white/5 flex justify-center">
                        <button onClick={handleTakeScreenshot} disabled={guardando} className="w-full md:w-auto bg-yellow-600 hover:bg-yellow-500 text-black px-12 py-4 rounded-2xl font-black shadow-xl flex items-center justify-center gap-3 uppercase text-sm active:scale-95 transition-all tracking-widest italic">
