...
export default function PBBuilder() {
    ...
    // ✅ REDISEÑO CANVAS HORIZONTAL ESTILO PRO
    const generateCanvas = async (isDownload = false) => {
        ...
        if (logo) {
            ...
            ctx.globalAlpha = 1.0;
            // Logo Cabecera
            ctx.drawImage(logo, 60, 45, 80, 80);
        }

        // 2. Textos Cabecera
        ctx.fillStyle = "#eab308";
        ctx.font = "bold 25px Arial";
        ctx.fillText("PRIMER BLOQUE", 160, 75);
        ctx.fillStyle = "white";
        ctx.font = "italic bold 85px Arial";
        ctx.fillText(nombreMazo.toUpperCase() || "MAZO SIN NOMBRE", 160, 155);

        // 3. Área de Cartas (Grid Horizontal)
        let x = 60, y = 220;
        ...
        for (const card of mazo) {
            ...
            x += 172;
            if (x > 1150) { x = 60; y += 235; }
        }

        // 4. Panel Curva de Oro (Derecha)
        const curveX = 1250;
        ...
        Object.entries(stats.curve).forEach(([cost, count]) => {
            ...
            ctx.fillRect(barX, 860 - 200, 30, 200); // Fondo
            ctx.fillStyle = "#eab308";
            ctx.fillRect(barX, 860 - barHeight, 30, barHeight); // Valor
            
            ctx.fillStyle = "white";
            ctx.font = "bold 16px Arial";
            ctx.fillText(cost, barX + 8, 895);
            ctx.font = "14px Arial";
            ctx.fillText(count, barX + 10, 650);
            barX += 45;
        });

        // 5. Panel Distribución (Inferior)
        ctx.fillStyle = "#1e293b";
        ctx.beginPath();
        ctx.roundRect ? ctx.roundRect(60, 970, 1150, 200, 25) : ctx.fillRect(60, 970, 1150, 200);
        ctx.fill();

        ctx.fillStyle = "white";
        ctx.font = "bold 24px Arial";
        ctx.fillText("Distribución del Mazo", 90, 1010);
        ...
        types.forEach(t => {
            ctx.fillStyle = "#334155";
            ctx.beginPath();
            ctx.roundRect ? ctx.roundRect(iconX, 1035, 180, 110, 15) : ctx.fillRect(iconX, 1035, 180, 110);
            ctx.fill();
            
            ctx.fillStyle = "#eab308";
            ctx.font = "bold 45px Arial";
            ctx.fillText(t.val, iconX + 65, 1090);
            ctx.fillStyle = "white";
            ctx.font = "16px Arial";
    ...
    useEffect(() => {
        if (modalMazoOpen) {
            const timer = setTimeout(() => generateCanvas(false), 300);
            return () => clearTimeout(timer);
        }
    }, [modalMazoOpen, mazo]);
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0c0e14] text-white overflow-hidden">
            ...
            {/* SIDEBAR DERECHO */}
            <div className="hidden md:flex w-85 border-l border-white/10 flex-col h-screen bg-slate-950 shadow-2xl">
            ...
            {modalMazoOpen && (
                <div className="fixed inset-0 bg-black z-[120] flex flex-col overflow-hidden animate-fade-in text-white">
                    <div className="p-4 bg-slate-900 flex justify-between items-center px-6 border-b border-orange-500/20 shadow-xl">
                        <h2 className="text-lg font-black uppercase text-yellow-500 italic flex items-center gap-2 tracking-tighter"><Layout size={20} /> Generador de Infografía</h2>
