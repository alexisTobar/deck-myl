...
export default function PBBuilder() {
    ...
    // ✅ FIX IMAGEN MÓVIL: Agregado delay y cacheBust para forzar renderizado de imágenes
    const handleTakeScreenshot = useCallback(async () => {
        if (!galleryRef.current) return;
        setGuardando(true);
        // Esperamos un momento para que el DOM "fuera de pantalla" se asiente
        ...
        try {
            const dataUrl = await toPng(galleryRef.current, { 
                quality: 1.0, 
                pixelRatio: 2,
                cacheBust: true, // Evita problemas de caché de imagen en móvil
            });
            const link = document.createElement('a');
            link.download = `WD_PB_${nombreMazo || "Deck"}.png`;
            link.href = dataUrl;
            link.click();
        } catch (err) { alert('Error al generar imagen'); } finally { setGuardando(false); }
    }, [nombreMazo]);
        ...
        <div className="h-screen flex flex-col md:flex-row font-sans bg-[#0c0e14] text-white overflow-hidden">
            ...
            <div className="hidden md:flex w-85 border-l border-white/10 flex-col h-screen bg-gradient-to-b from-slate-900 via-[#0c0e14] to-black shadow-2xl">
                ...
                <div className="p-5 bg-slate-900/80 backdrop-blur-xl border-t border-white/5 flex flex-col gap-3 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
                    ...
                    <button onClick={() => setModalGuardarOpen(true)} className="w-full bg-yellow-600 hover:bg-yellow-500 text-black py-3 rounded-2xl font-black text-[11px] uppercase flex items-center justify-center gap-2 shadow-xl active:scale-95 transition-all"><Save size={16} /> Guardar Mazo</button>
                </div>
            </div>
            ...
            {modalMazoOpen && (
                <div className="fixed inset-0 bg-black z-[120] flex flex-col overflow-hidden animate-fade-in text-white">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 pb-24 md:p-10 bg-[#0c0e14] flex flex-col items-center">
                        {/* VISTA PREVIA MÓVIL (VISIBLE EN PANTALLA) */}
                        ...
                        <div className="hidden md:block">
                            <div ref={galleryRef} className="relative w-[1200px] flex flex-col bg-[#0c0e14] p-10 border-4 border-yellow-600 shadow-2xl overflow-hidden rounded-sm">
                                ...
                                <div className="grid grid-cols-5 gap-3 relative z-10">
                                    {TIPOS_PB.map(t => (
                                        <div key={t.id} className="flex flex-col items-center justify-center bg-slate-900 p-6 rounded-3xl border border-yellow-500/20 shadow-inner">
                        ...
                        {/* Clon para captura móvil (FUERA DE PANTALLA) */}
                        <div className="md:hidden" style={{ position: 'fixed', top: '-10000px', left: '-10000px' }}>
                             <div className="relative w-[1200px] flex flex-col bg-[#0c0e14] p-10 border-4 border-yellow-600 shadow-2xl overflow-hidden rounded-sm" 
                                 ...
                                 <div className="flex justify-between items-end mb-8 border-b-2 border-yellow-500/20 pb-4 text-white font-black italic">
                                    <div><h1 className="text-6xl uppercase tracking-tighter leading-none mt-1">{nombreMazo}</h1></div>
                                 ...
                                 <div className="grid grid-cols-5 gap-3">
                                    {TIPOS_PB.map(t => (
                                        <div key={t.id} className="flex flex-col items-center justify-center bg-slate-900 p-6 rounded-3xl border border-yellow-600/20 shadow-inner">
                                            <span className="text-4xl font-black text-white">{statsForExport.counts[t.label] || 0}</span>
                                            <span className="text-[8px] uppercase font-black text-slate-500 mt-1">{t.label}</span>
                                        </div>
                                    ))}
            ...
            {cardToZoom && (
                <div className="fixed inset-0 z-[200] bg-black/95 backdrop-blur-md flex flex-col items-center justify-center p-4" onClick={() => setCardToZoom(null)}>
