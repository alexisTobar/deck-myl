...
export default function CommunityPB() {
    ...
    // ✅ FIJAMOS EL FORMATO A PB
    const activeFormat = "primer_bloque";
    ...
    // ✅ Lógica para abrir mazo automáticamente desde el Home
    useEffect(() => {
    ...
    const fetchDecks = async () => {
        try {
            ...
            console.error("Error cargando comunidad PB:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleClone = (deck) => {
        navigate('/primer-bloque/builder', { state: { deckToEdit: deck, isCloning: true } });
    };

    const getImg = (c) => c?.imgUrl || c?.imageUrl || c?.img || "https://via.placeholder.com/250x350?text=No+Image";

    // ✅ Solo mostramos Primer Bloque
    const filteredDecks = useMemo(() => {
        return decks.filter(d => d.format === "primer_bloque");
    }, [decks]);
    ...
    if (loading) return <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] flex items-center justify-center"><div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div></div>;
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] text-slate-900 dark:text-white pb-32 font-sans overflow-x-hidden transition-colors duration-500">
            ...
            <div className="bg-white/70 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/5 sticky top-0 z-30 px-4 py-3 md:px-6 md:py-4">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-blue-600 rounded-xl shadow-lg shadow-blue-600/20"><Globe size={18} className="text-white" /></div>
                        <h1 className="text-lg md:text-xl font-black tracking-tighter uppercase italic">Arena PB <span className="text-blue-600">Comunidad</span></h1>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 mt-8 md:mt-16">
                ...
                {topDecks.length > 0 && (
                    <div className="mb-20">
                        <div className="flex flex-col items-center mb-10">
                            ...
                            <h2 className="text-2xl md:text-3xl font-black uppercase italic text-center">Top <span className="text-blue-600">Primer Bloque</span></h2>
                        </div>
                ...
                <h2 className="text-lg font-black flex items-center gap-3 uppercase italic mb-8"><Flame className="text-blue-600" size={18} /> Mazos Recientes</h2>
            ...
            {/* Modal Detalle (El mismo de tu Community original) */}
            {selectedDeck && (
                <div className="fixed inset-0 z-[110] bg-slate-950/95 flex items-end md:items-center justify-center transition-all" onClick={() => setSelectedDeck(null)}>
                    <div className="bg-white dark:bg-slate-900 w-full max-w-7xl h-[95vh] md:h-[90vh] rounded-t-[2.5rem] md:rounded-[2.5rem] flex flex-col md:flex-row overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                        {/* ... Contenido del modal (Mismo que Community) ... */}
                        <div className="flex-[3] flex flex-col min-h-0 border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/5">
                            <div className="p-5 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                                <div><h2 className="text-xl md:text-3xl font-black uppercase italic">{selectedDeck.name}</h2><p className="text-[10px] uppercase text-slate-400">Por: @{selectedDeck.user?.username}</p></div>
                                <div className="flex gap-2">
                                    <button onClick={() => handleClone(selectedDeck)} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase"><Copy size={14} /></button>
                                    <button onClick={() => setSelectedDeck(null)} className="p-2 text-slate-500"><X size={18} /></button>
                                </div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-4">
                                    {selectedDeck.cards.map((c, i) => (
                                        <div key={i} className="relative"><img src={getImg(c)} className="w-full rounded-xl" /><div className="absolute -bottom-1 -right-1 bg-blue-600 text-white w-6 h-6 flex items-center justify-center text-[10px] font-black rounded-lg">x{c.quantity}</div></div>
                                    ))}
                                </div>
                            </div>
                        </div>
                        {/* Comentarios */}
                        <div className="flex-[1.2] flex flex-col bg-white dark:bg-slate-800/20">
                             <div className="p-5 border-b font-black text-[10px] uppercase text-blue-600">Comentarios</div>
                             <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">
                                {selectedDeck.comments?.map((com, idx) => (
                                    <div key={idx} className="bg-slate-50 dark:bg-black/30 p-3 rounded-xl border dark:border-white/5">
                                        <p className="text-[9px] font-black text-blue-500">@{com.username}</p>
                                        <p className="text-xs">{com.text}</p>
                                    </div>
                                ))}
                             </div>
                             <div className="p-5 border-t"><div className="flex gap-2"><input type="text" placeholder="Comentar..." value={newComment} onChange={e => setNewComment(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleAddComment()} className="flex-1 bg-slate-100 dark:bg-slate-800 border-0 rounded-xl px-3 py-2 text-xs" /><button onClick={handleAddComment} className="bg-blue-600 p-2 rounded-xl text-white"><Send size={16} /></button></div></div>
                        </div>
                    </div>
                </div>
            )}
...
function RankingPodiumItem({ deck, rank, color, height, userId, onLike, onClick, getImg, isWinner }) {
    return (
        <div className={`flex flex-col items-center group w-full md:w-auto ${isWinner ? 'z-20 md:scale-105' : 'z-10'}`}>
            <div onClick={onClick} className={`relative w-[85%] md:w-52 lg:w-60 overflow-hidden rounded-t-[2.5rem] border-x-4 border-t-4 transition-all cursor-pointer shadow-2xl ${isWinner ? 'border-blue-600' : 'border-slate-200 dark:border-white/10'}`}>
                <div className={`${height} relative bg-white dark:bg-slate-900`}>
                    ...
                    <div className="absolute top-4 left-6 w-10 h-10 flex items-center justify-center rounded-xl bg-white dark:bg-slate-800 font-black shadow-xl">{rank}</div>
                    <div className="absolute bottom-3 left-0 w-full px-4 text-center"><h3 className="font-black text-sm uppercase italic truncate">{deck.name}</h3><p className="text-blue-600 text-[8px] font-black uppercase">@{deck.user?.username}</p></div>
                </div>
            </div>
            <button onClick={(e) => onLike(deck._id, e)} className={`w-[85%] md:w-52 lg:w-60 h-12 ${color} flex items-center justify-center rounded-b-2xl border-t border-white/10`}>
    ...
    return (
        <div onClick={onClick} className="group bg-white dark:bg-slate-900 rounded-[1.8rem] border border-slate-200 dark:border-white/10 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col shadow-sm">
            <div className="h-32 md:h-40 relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                ...
                <div className="absolute bottom-2 px-3 w-full"><h4 className="text-[10px] md:text-xs font-black uppercase italic truncate leading-none">{deck.name}</h4></div>
            </div>
            <div className="p-3 md:p-4 flex flex-col gap-3">
                <div className="flex justify-between items-center"><span className="text-[8px] font-black text-blue-600 uppercase truncate max-w-[80px]">@{deck.user?.username}</span><button onClick={(e) => onLike(deck._id, e)} className="flex items-center gap-1"><Heart size={14} className={deck.likes?.includes(userId) ? "fill-red-500 text-red-500" : "text-slate-300"} /><span className="text-[9px] font-black text-slate-400">{deck.likes?.length || 0}</span></button></div>
            </div>
        </div>
    );
}