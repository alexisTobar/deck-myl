...
// ✅ Definición de las 4 Ediciones Principales
...
export default function AdminCards() {
    ...
    const initialFormState = {
        ...
        main_edition: "" // ✅ Estado inicial para que React lo controle
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // ✅ AQUÍ ESTÁ LA CORRECCIÓN: Forzamos la inclusión de main_edition en el objeto que se envía
        const dataToSend = { 
            ...formData, 
            edition_slug: formData.edition, 
            img: formData.imgUrl,
            // Si es primer bloque, enviamos el valor del select, si no, vacío
            main_edition: formato === "primer_bloque" ? formData.main_edition : ""
        };
        ...
        try {
            ...
            if (res.ok) {
                alert("¡Base de datos actualizada! La carta ahora tiene Edición Principal. ⚔️");
                resetForm();
                fetchCartas();
    ...
    const resetForm = () => {
        setEditingCard(null);
        setFormData({ ...initialFormState, format: formato, edition: edicionFiltro, main_edition: "" });
    };

    if (step === "selector") {
        return (
            <div className="min-h-screen bg-[#0B1120] flex flex-col items-center justify-center p-4">
                <h1 className="text-4xl font-black text-white mb-10 uppercase italic tracking-tighter">Gestión de Armería</h1>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl">
                    <button onClick={() => handleSelectFormat("imperio")} className="group bg-slate-900 border-2 border-orange-500/20 p-12 rounded-[3rem] hover:border-orange-500 transition-all shadow-2xl active:scale-95">
                        <span className="text-7xl mb-4 block">🏛️</span>
                        <h2 className="text-2xl font-black text-white uppercase italic">Imperio</h2>
                    </button>
                    <button onClick={() => handleSelectFormat("primer_bloque")} className="group bg-slate-900 border-2 border-yellow-500/20 p-12 rounded-[3rem] hover:border-yellow-500 transition-all shadow-2xl active:scale-95">
    ...
    return (
        <div className="min-h-screen bg-[#0B1120] text-white pb-32 font-sans selection:bg-orange-500">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    ...
                    <div className={`lg:col-span-1 bg-slate-900/95 p-8 rounded-[2.5rem] border border-white/5 shadow-2xl backdrop-blur-md transition-all ${showFormMobile ? 'fixed inset-4 z-[120] overflow-y-auto pb-20' : 'hidden lg:block h-fit sticky top-24'}`}>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            {/* ✅ SELECTOR DE EDICIÓN PRINCIPAL (Solo se muestra en PB) */}
                            ...
                            <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase ml-2 mb-1 block">Edición Base (Carpeta)</label>
                                <select className="w-full p-3.5 bg-slate-800 rounded-2xl border border-white/5 outline-none text-xs font-black" value={formData.edition} onChange={e => setFormData({...formData, edition: e.target.value})} required>
                                    {Object.entries(formato === "imperio" ? EDICIONES_IMPERIO : EDICIONES_PB).map(([slug, label]) => (<option key={slug} value={slug}>{label}</option>))}
                                </select>
                            </div>
                            ...
                            {formato === "primer_bloque" && (
                                <div>
                                    <label className="text-[9px] font-black text-slate-500 uppercase ml-2 block">Raza</label>
                            ...
                            <div>
                                <label className="text-[9px] font-black text-slate-500 uppercase ml-2 block">URL Imagen</label>
                            ...
                            <div className="flex gap-2 pt-6">
                                ...
                                {editingCard && <button type="button" onClick={resetForm} className="bg-slate-700 px-6 rounded-2xl font-black transition-all">✕</button>}
                            </div>
                        </form>
                    </div>
                    ...
                    <div className="lg:col-span-3">
                        {loading ? (
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-6 overflow-y-auto max-h-[80vh] p-2 custom-scrollbar">
                                {cartas.map(c => (
                                    <div key={c._id} className="bg-slate-900 p-2.5 rounded-[2rem] border border-white/5 group relative overflow-hidden shadow-xl hover:border-orange-500/50 transition-all">
                                        {/* ✅ Icono visual si ya tiene edición principal asignada */}
                                        {c.main_edition && (
                                            <div className="absolute top-4 right-4 z-10 bg-yellow-500 text-black p-1.5 rounded-lg shadow-2xl flex items-center gap-1 font-black text-[8px] uppercase italic border border-black/10">
                                                <Star size={10} fill="currentColor"/> {c.main_edition.replace('_', ' ')}
                                            </div>
                                        )}
                                        ...
                                        <div className="mt-3 px-2 text-center pb-1">
                                            ...
                                            <p className="text-[9px] text-slate-500 font-bold uppercase">{c.slug}</p>
                                        </div>
                                        <div className="absolute inset-0 bg-slate-950/90 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center gap-3 transition-all duration-300 backdrop-blur-sm">
                                            <button onClick={() => {
                                                setEditingCard(c);
                                                // ✅ PASAR EL main_edition AL FORMULARIO AL EDITAR
                                            ...
                                            }} className="w-28 bg-blue-600 py-3 rounded-full text-[10px] font-black uppercase shadow-lg active:scale-95">Editar</button>
                                            <button onClick={async () => { if(window.confirm("¿Borrar permanentemente?")) { await fetch(`${BACKEND_URL}/api/cards/${c._id}`, { method: "DELETE", headers: { "auth-token": token } }); fetchCartas(); } }} className="w-28 bg-red-600 py-3 rounded-full text-[10px] font-black uppercase shadow-lg active:scale-95">Borrar</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
