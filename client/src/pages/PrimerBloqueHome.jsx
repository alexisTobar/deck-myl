...
export default function PrimerBloqueHome() {
    ...
    return (
        <div className="min-h-screen bg-[#060912] text-white font-sans overflow-x-hidden selection:bg-yellow-500 relative">
            ...
            <section className="relative h-screen flex items-center justify-center border-b border-yellow-500/10 z-10 px-4">
                <div className="text-center max-w-6xl">
                    ...
                    <motion.h1 
                        ...
                        className="text-7xl md:text-[11rem] font-black text-white mb-6 uppercase tracking-tighter italic leading-none drop-shadow-[0_0_40px_rgba(234,179,8,0.4)]"
                    >
                        PRIMER BLOQUE
                    </motion.h1>
                    ...
                    <motion.div {...fadeInUp} className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                        <button onClick={() => setShowModal(true)} className="group relative px-14 py-7 bg-yellow-600 rounded-2xl transition-all hover:scale-110 active:scale-95 shadow-2xl shadow-yellow-600/20 overflow-hidden text-black">
                            <span className="relative z-10 font-black uppercase italic text-2xl flex items-center gap-3">
                                <Sword size={28} /> ENTRAR A LA FORJA
                            </span>
                        </button>
                        <Link to="/community" className="px-14 py-7 bg-slate-900/60 backdrop-blur-2xl border border-white/10 rounded-2xl font-black transition-all hover:bg-yellow-600/10 flex items-center justify-center gap-3 uppercase italic text-2xl text-slate-200">
            ...
            <section className="bg-slate-950/50 py-32 border-y border-white/10 relative z-10">
                <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12">
                    <ResourceBox 
                        title="Banlist Racial"
                        desc="Revisa las restricciones de cartas para cada raza en PB Extendido y mantén el equilibrio de tus duelos."
                        icon={<Trophy size={48} />}
            ...
            <footer className="bg-black py-20 border-t border-white/5 relative z-10 text-center md:text-left">
                <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-12">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm font-bold uppercase tracking-[0.3em] mb-6">Resguardando la Llama Clásica</p>
                        <div className="flex justify-center md:justify-start gap-8">
                            <a href="#" className="text-slate-400 hover:text-yellow-500 transition-all scale-125"><Instagram size={24}/></a>
                            <a href="#" className="text-slate-400 hover:text-yellow-500 transition-all scale-125"><Youtube size={24}/></a>
                            <a href="#" className="text-slate-400 hover:text-yellow-500 transition-all scale-125"><Twitter size={24}/></a>
                        </div>
                    </div>
                    <p className="text-[10px] text-slate-800 font-black uppercase tracking-[0.5em]">WarningDeck © 2025 • PB Extendido</p>
                </div>
            </footer>
            ...
            <AnimatePresence>
                {showModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm">
                        <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-[#0c111d] border border-yellow-500/30 w-full max-w-2xl rounded-[3rem] p-10 relative">
                            ...
                            <h3 className="text-4xl font-black uppercase italic tracking-tighter text-yellow-500 text-center mb-10 text-yellow-500">Selecciona tu Edición</h3>
...
// Subcomponentes (Reusando lógica de colores con clases dinámicas si fuera necesario, aquí separados por comodidad)
function RaceRank({ name, power, trend, color }) {
    return (
        <div className="bg-white/5 p-6 rounded-2xl border border-white/5 flex items-center justify-between group hover:border-yellow-500/50 transition-all text-yellow-500">
            ...
            <div className="text-right text-yellow-500 font-black text-2xl">{power}</div>
        </div>
    );
}
