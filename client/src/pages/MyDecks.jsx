...
export default function MyDecks() {
    ...
    const confirmDelete = async () => {
        ...
        } catch (err) { showToast("Error", "error"); } finally { setDeckToDelete(null); }
    };

    const handleEdit = (deck) => {
        const styles = getFormatStyles(deck.format);
        // Usamos navigate para movernos entre componentes sin recargar la página
    ...
    const getCardImage = (cards) => cards?.length ? (cards[0].imgUrl || cards[0].imageUrl || cards[0].img) : null;
    ...
    if (loading) return <div className="min-h-screen bg-slate-900 flex items-center justify-center text-white text-xl animate-pulse">Cargando la armería... ⚔️</div>;
    ...
    return (
        <div className="min-h-screen bg-slate-900 font-sans text-slate-200 pb-20">
            ...
            <div className="bg-slate-800 border-b border-slate-700 sticky top-0 z-30 shadow-lg backdrop-blur-md bg-opacity-90">
                <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto bg-slate-900 p-1.5 rounded-xl border border-slate-700">
                        ...
                        <select className="bg-slate-800 text-sm text-slate-300 rounded-lg px-3 py-2 outline-none cursor-pointer w-full sm:w-auto" value={filterFormat} onChange={(e) => setFilterFormat(e.target.value)}>
                            <option value="all">📚 Todos</option>
                            <option value="imperio">🏛️ Imperio</option>
                            <option value="primer_bloque">📜 Primer Bloque</option>
                        </select>
                        <select className="bg-slate-800 text-sm text-slate-300 rounded-lg px-3 py-2 outline-none cursor-pointer w-full sm:w-auto" value={sortBy} onChange={(e) => setSortBy(e.target.value)}>
                ...
                {processedDecks.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        {processedDecks.map((deck) => {
                            const formatStyle = getFormatStyles(deck.format);
                            return (
                                <div key={deck._id} onClick={() => setSelectedDeck(deck)} className="group relative bg-slate-800 rounded-2xl overflow-hidden border border-slate-700 hover:border-orange-500 transition-all cursor-pointer h-72 flex flex-col shadow-lg">
                                    ...
                                    <div className="absolute top-3 left-3 z-20">
                                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full border backdrop-blur-md ${formatStyle.badgeClass}`}>{formatStyle.label}</span>
                                    </div>
                ...
                <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={() => setSelectedDeck(null)}>
                    <div className="bg-slate-800 w-full max-w-4xl rounded-2xl shadow-2xl border border-slate-600 flex flex-col max-h-[90vh] overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="p-6 border-b border-slate-700 flex justify-between bg-slate-900/50">
                            <div>
                                ...
                                <div className="flex gap-3 mt-2">
                                    <span className={`text-xs font-bold px-2 py-1 rounded border ${getFormatStyles(selectedDeck.format).badgeClass}`}>{getFormatStyles(selectedDeck.format).label}</span>
                        ...
                        <div className="p-4 border-t border-slate-700 bg-slate-800 grid grid-cols-2 md:grid-cols-4 gap-3">
                            <button onClick={() => togglePrivacy(selectedDeck)} className="flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 text-white py-2.5 rounded-lg font-bold transition"><IconLock /> {selectedDeck.isPublic ? "Privado" : "Público"}</button>
                            <button onClick={() => handleExport(selectedDeck)} className="flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 rounded-lg font-bold transition"><IconDownload /> Exportar</button>
                            <button onClick={() => handleEdit(selectedDeck)} className="flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-500 text-white py-2.5 rounded-lg font-bold transition"><IconEdit /> Editar</button>
                            <button onClick={() => setDeckToDelete(selectedDeck)} className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-500 text-white py-2.5 rounded-lg font-bold transition"><IconTrash /> Eliminar</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {/* TOAST */}
            {toast.show && <div className={`fixed bottom-24 md:bottom-10 left-1/2 -translate-x-1/2 px-6 py-3 rounded-2xl shadow-2xl z-[100] font-bold ${toast.type === "error" ? "bg-red-600" : "bg-emerald-600"} text-white`}>{toast.msg}</div>}
        </div>
    );
}