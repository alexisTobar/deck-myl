...
export default function MyDecks() {
    ...
    const [showMobileComments, setShowMobileComments] = useState(false); // ✅ Control de comentarios en móvil
    ...
    return (
        <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0B1120] text-slate-900 dark:text-white pb-32 transition-colors duration-500 overflow-x-hidden font-sans">
            ...
            <div className="max-w-7xl mx-auto p-6 md:p-10">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                    {processedDecks.map((deck) => (
                        <div key={deck._id} onClick={() => setSelectedDeck(deck)} className="group relative bg-white dark:bg-slate-900 rounded-[2rem] md:rounded-[2.5rem] overflow-hidden border border-slate-200 dark:border-white/10 hover:shadow-2xl transition-all duration-500 cursor-pointer flex flex-col shadow-sm">
                            <div className="h-36 md:h-48 relative overflow-hidden bg-slate-100 dark:bg-slate-800">
                                ...
                                <div className="absolute top-3 left-3 z-10">
                                    <span className={`text-[7px] md:text-[8px] font-black px-2 py-0.5 rounded-full uppercase ${getFormatStyles(deck.format).badgeClass}`}>{getFormatStyles(deck.format).label}</span>
                                </div>
                            </div>
            ...
            {/* --- MODAL DETALLE (OPTIMIZADO MÓVIL) --- */}
            {selectedDeck && (
                <div className="fixed inset-0 z-[110] bg-slate-950/95 md:backdrop-blur-md flex items-end md:items-center justify-center transition-all" onClick={() => { setSelectedDeck(null); setShowMobileComments(false); }}>
                    <div className="bg-white dark:bg-slate-900 w-full max-w-7xl h-[95vh] md:h-[90vh] rounded-t-[2.5rem] md:rounded-[3rem] border-x border-t md:border border-slate-200 dark:border-white/10 flex flex-col overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        {/* Cabecera Responsiva */}
                        <div className="p-5 md:p-10 border-b border-slate-100 dark:border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-shrink-0 relative">
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-3 md:gap-8">
                                    {selectedDeck.cards.map((c, i) => (
                                        <div key={i} className="relative group animate-in fade-in zoom-in-95">
                                            ...
                                            <p className="mt-1.5 text-[7px] md:text-[10px] text-slate-400 dark:text-slate-500 font-black uppercase truncate text-center italic">{c.name}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
