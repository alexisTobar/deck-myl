...
export default function DeckBuilder() {
    ...
    const [isPublic, setIsPublic] = useState(false); // <--- NUEVO ESTADO: Público/Privado
    ...
    // 🔥 MEJORA 1: DETECTAR CAMBIO DE EDICIONES Y SELECCIONAR LA PRIMERA AUTOMÁTICAMENTE
    useEffect(() => {
        const claves = Object.keys(edicionesActivas);
        if (claves.length > 0) {
            // Selecciona automáticamente la primera edición del objeto
            setEdicionSeleccionada(claves[0]);
        } else {
    ...
    const cambiarFormato = (nuevoFormato) => {
        ...
        setBusqueda("");
        setCartas([]); 
        // edicionSeleccionada se actualiza sola gracias al useEffect de arriba
    };
    ...
    useEffect(() => {
        if (location.state && location.state.deckToEdit) {
            ...
            if(deck.isPublic !== undefined) setIsPublic(deck.isPublic); // <--- CARGAMOS ESTADO PUBLICO
    ...
    useEffect(() => {
        const fetchCartas = async () => {
            // 🔥 Si no hay edición ni búsqueda, no cargar nada para no saturar
            ...
            try {
                ...
                params.append("format", formato); // 🔥 IMPORTANTE: Envía el formato al backend
        ...
        try {
            ...
            const res = await fetch(url, {
                ...
                body: JSON.stringify({ 
                    name: nombreMazo, 
                    cards: cardsPayload,
                    format: formato, // Enviar formato
                    isPublic: isPublic // <--- MEJORA 2: ENVIAR SI ES PÚBLICO O NO
                })
            });

            if (res.ok) {
                alert(isPublic ? "¡Mazo guardado y PUBLICADO! 🌍" : "¡Mazo guardado en privado! 🔒");
                navigate("/my-decks");
            } else {
                const data = await res.json();
    ...
    return (
        <div className="h-screen flex flex-col md:flex-row font-sans bg-slate-900 text-white overflow-hidden">
                ...
                <div className="bg-slate-900 border-b border-slate-800 p-2 z-30 flex items-center gap-2 shadow-md">
                    ...
                    <div className="hidden md:flex gap-2 ml-2">
                        <select value={edicionSeleccionada} onChange={(e) => setEdicionSeleccionada(e.target.value)} className="p-2 rounded-lg bg-slate-800 border border-slate-700 text-xs hover:border-orange-500 cursor-pointer max-w-[150px]">
                            {/* Si no hay nada seleccionado, muestra el placeholder, pero el useEffect lo cambiará rápido */}
            ...
            {/* MODAL GUARDAR CON OPCIÓN DE PÚBLICO (AQUÍ ESTÁ LA MEJORA 2) */}
            {modalGuardarOpen && (
                <div className="fixed inset-0 bg-black/90 z-[110] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-slate-800 p-6 rounded-2xl w-full max-w-sm border border-slate-700 animate-fade-in">
                        ...
                        <div className="flex items-center gap-3 mb-6 bg-slate-900 p-3 rounded-lg border border-slate-700">
                            ...
                            />
                            <label htmlFor="isPublicCheck" className="cursor-pointer text-sm text-slate-300 select-none font-bold">
